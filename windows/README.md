...
开始连接成功后会自动全屏，依次进行设备检查、睁眼基线、说明、练习和正式采集。按 Esc 会结束并保存，不只是退出全屏。结束后自动恢复参数界面。

设备检查现采用宽松采集规则：偶发通信间隔、时间戳异常、平直等会在底部提示并写入事件文件，仍可继续；持续 10 秒没有新数据、录制故障，或 30 秒检查结束仍不足 100 个样本才中止。若一点击就提示“连接失败”，请反馈完整文字：流通道数、FP1/FP2 标签和初始时钟连接检查仍保留。采集完成不代表数据可训练，训练会单独检查质量。
